...
import {ArrowLeft,ShieldCheck,Users,UserCheck,UserX,MessageCircle,UserPlus} from "lucide-react";
import "./admin.css";
...
export default function Yonetim(){
 ...
 <div className="memberList">{members.map(m=><article className="member" key={m.id}><div className="identity"><div className="pic">{m.avatar_url?<img src={m.avatar_url} alt=""/>:(m.full_name||"Ü").slice(0,1)}</div><div><b>{m.full_name}</b><span>@{m.username||"uye"}</span></div></div><div className="badges"><i className={m.role}>{m.role==="admin"?"ADMIN":m.role==="moderator"?"MODERATÖR":"ÜYE"}</i><i className={m.is_active?"on":"off"}>{m.is_active?"AKTİF":"PASİF"}</i></div><div className="actions"><select value={m.role} disabled={m.id===me.id} onChange={e=>setRole(m.id,e.target.value)}><option value="member">Üye</option><option value="moderator">Moderatör</option><option value="admin">Admin</option></select><button disabled={m.id===me.id} onClick={()=>setActive(m.id,!m.is_active)}>{m.is_active?"Pasife Al":"Aktifleştir"}</button></div></article>)}</div></section></main>
}