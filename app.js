...
function render(){
 ...
 renderDues();renderExpenses();renderMembers();renderLogs();
}
function renderDues(){
 const p=D.periods.find(x=>+x.id===+periodId);$('#periods').innerHTML=D.periods.map(x=>`<button class="period ${+x.id===+periodId?'selected':''}" data-p="${x.id}"><b>${esc(x.name)} ${x.year}</b><small>Aidat tutarı</small><input data-a="${x.id}" type="number" value="${x.amount}"></button>`).join('');
 if(!p){$('#duesTable').innerHTML='Aidat dönemi yok';return}
 $('#duesTable').innerHTML=`<table><thead><tr><th>Üye</th><th>Tutar</th><th>Ödenen</th><th>Durum</th><th></th></tr></thead><tbody>${D.members.map(m=>{let x=pay(m.id,p.id),s=x?.status||'unpaid';return `<tr><td>${esc(m.name)}</td><td>${money(p.amount)}</td><td>${money(x?.amount||0)}</td><td><span class="badge ${s==='paid'?'paid':'unpaid'}">${s==='paid'?'ÖDENDİ':'ÖDENMEDİ'}</span></td><td><button data-edit="${m.id}">Düzenle</button></td></tr>`}).join('')}</tbody></table>`;
 document.querySelectorAll('[data-p]').forEach(b=>b.onclick=e=>{if(e.target.tagName==='INPUT')return;periodId=+b.dataset.p;renderDues()});
 document.querySelectorAll('[data-a]').forEach(i=>{i.onclick=e=>e.stopPropagation();i.onchange=async()=>{await run('periods',q=>q.update({amount:+i.value||0}).eq('id',i.dataset.a));await log('Aidat tutarı güncellendi',i.value+' TL');load()}});
